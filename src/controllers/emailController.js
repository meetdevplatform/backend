...
const { genJWTToken, encryptString, encryptObject} = require('../common/helpers/auth')
    ...
    try {
        ...
        if (!isEmpty(existedUser)) {
            ...
            if (response.statusCode === STATUS_CODE.SUCCESS) {
                response.data = await User.findOneAndUpdate({ email: `${email.toLocaleLowerCase()}` }, valueUpdate)
            }
        } else {
            return response
        }
...
const verifyEmail = async (req, res) => {
    ...
    if (response.statusCode === STATUS_CODE.SUCCESS) {
        const token = await genJWTToken(`${response.data._id}`, VARIABLE.USER_TOKEN_EXPIRED)
